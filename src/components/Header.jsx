...
export function Header() {
  ...
  return (
    <header
      className={`p-4 px-7 h-[80px] ${textLightMenu} ${bgDarkColor}  absolute right-0 left-0`}
    >
      <nav className={`flex items-center justify-between container mx-auto`}>
        ...
        <ul className={`lg:flex gap-5 sm:opacity-100 ${active ? " px-5 absolute top-0 right-0 left-0 mt-[80px] w-full h-auto pb-4 opacity-90 sm:pb-0 bg-[#0F0D0E] duration-300" : "opacity-0 h-0"}`}>
          <li>
            <a
        ...
        <div className={`${active ? "flex" : "hidden"} sm:flex items-center gap-2`}>
          <button
